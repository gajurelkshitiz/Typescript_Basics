...
const newUser: User = [113, "example@google.com"];
...
const testArray: [number, string] = [10, "test"];        // This works.
testArray.push('s');    // It shouldn't had worked, but it works.


    // In order to fix this: we need to define a scrictTuple:

type strictTuple <T extends any[]> = 
    Omit <T, keyof (any[])> extends infer O
        ? { [k in keyof O] : O[k] }
        : never

type myTuple = [string, number];
type step3 = Omit<myTuple, keyof any[]>

let item: step3 = ["hello", 1]

// item.push() // this doesn't exist here
